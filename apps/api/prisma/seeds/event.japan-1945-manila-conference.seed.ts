...
<p>이에지마에서 환승된 이유는 ① 일본 항공기의 항속거리(2,000km) 한계, ② 미군 측의 안전·정보보안 요구 양쪽이었다. 환승 절차에서 사절단의 휴대 문서·필기구·시계까지 모두 미군 보안 검사를 거쳤으며, 가와베 단장은 회고록에서 <em>"기왓장 한 장의 무게도 우리 어깨에 앉은 듯 느꼈다"</em>고 기술했다.</p>`,
    },
    {
      order: 3,
...
<p>맥아더 SCAP은 회담장에 직접 등장하지 않았다. 이는 ① 의도된 거리두기로 위계를 명확히 하고, ② 합의 사항이 SCAP의 결재(裁可)를 한 단계 거쳐야 발효된다는 점을 일본 측에 인식시키기 위한 연출이었다(서덜랜드 회고록·Willoughby Papers 기록).</p>
...
<ol>
  ...
  <li><strong>서명자 지정 요구</strong>: 일본 측은 ① 정부 대표(외무대신 시게미쓰 마모루로 결정될 예정), ② 군 대표(참모총장 우메즈 요시지로). 미국 측 대표 외에 영국·소련·중국·캐나다·호주·뉴질랜드·네덜란드·프랑스 8개국이 추가 서명.</li>
