...
/**
 * 배우자 혼인일 입력 → 구조화(era + 크기값 연/월/일 + precision) + 저장가능 DateTime.
 * 구조화 값(marriageStart/End)이 있으면 우선, 없으면 레거시 ISO 문자열을 직접 파싱
 * (네이티브 new Date() 금지 — BC '-0044-…'가 AD 2044로 둔갑). 값이 없으면 전부 null(행 단위 절대값).
 * DateTime은 AD 1000~9999만 채운다 — MySQL DATETIME 공식 범위 밖은 드라이버 직렬화에서
 * 2자리 연도 규칙(44→2044)·pre-standard TZ 드리프트로 조용히 손상됨을 실측 확인(구조화 필드가 진실).
 */
function mapSpouseDateInput(
  structured: { era: 'BC' | 'AD'; year: number; month?: number; day?: number } | null | undefined,
  ...
  const fromParts = (era: 'BC' | 'AD', year: number, month?: number, rawDay?: number) => {
  ...
  if (structured) {
    return fromParts(structured.era, structured.year, structured.month, structured.day)
  }
  if (legacyIso) {
...
/** 혼인 서열 정렬 순서 — enum 정의 순서(정실 우선) */
const MARRIAGE_RANK_ORDER: Record<string, number> = {
  PRIMARY: 0,
  SECONDARY: 1,
  CONCUBINE: 2,
  MORGANATIC: 3,
...
export class PersonController {
  ...
  async getDetailById(@Param('id') id: string, @Request() req: any): Promise<{
    ...
    return {
      ...
      governmentPositions: serializeBigInt(person.GovernmentTenures ?? []),
      spouseRelations: (() => {
        const mapRel = (rel: any, spousePerson: any) => ({
          id: rel.id,
          marriageStartDate: rel.marriageStartDate?.toISOString?.() ?? null,
          marriageStartEra: rel.marriageStartEra ?? null,
          marriageStartYear: rel.marriageStartYear ?? null,
          marriageStartMonth: rel.marriageStartMonth ?? null,
          marriageStartDay: rel.marriageStartDay ?? null,
          marriageStartPrecision: rel.marriageStartPrecision ?? null,
          marriageEndDate: rel.marriageEndDate?.toISOString?.() ?? null,
          marriageEndEra: rel.marriageEndEra ?? null,
          marriageEndYear: rel.marriageEndYear ?? null,
          marriageEndMonth: rel.marriageEndMonth ?? null,
          marriageEndDay: rel.marriageEndDay ?? null,
          marriageEndPrecision: rel.marriageEndPrecision ?? null,
          marriageRank: rel.marriageRank ?? null,
          note: rel.note ?? null,
          spouse: spousePerson ? serializeBigInt(spousePerson) : null,
        })
        // 혼인 시작 부호 연도(BC 음수) — 구조화 연도 우선(DateTime은 AD 1000+ 전용), 레거시는 ISO 연도
        const signedStartYear = (rel: {
          ...
          const year =
            ...
            (rel.marriageStartDate ? parseInt(rel.marriageStartDate.slice(0, 4), 10) : NaN)
          if (year == null || isNaN(year)) return Number.POSITIVE_INFINITY
          return rel.marriageStartEra === 'BC' ? -year : year
        }
        // 배우자별 dedup — 시드 유래 양방향 중복행에서 "먼저 온 빈 행"이 canonical 행의
        // 서열·혼인일을 가리지 않도록, 메타가 더 풍부한 행을 대표로 채택한다.
        const relRichness = (rel: {
          marriageStartYear: number | null
          marriageStartDate: string | null
          ...
          [
            rel.marriageStartYear != null || rel.marriageStartDate,
            rel.marriageEndYear != null || rel.marriageEndDate,
            rel.marriageRank,
            rel.note,
          ].filter(Boolean).length
        const bySpouseId = new Map<string, ReturnType<typeof mapRel>>()
        const noSpouseRows: Array<ReturnType<typeof mapRel>> = []
        for (const rel of [
          ...(person.spouseRelationsAsPerson || []).map((r: any) => mapRel(r, r.spouse)),
          // 역방향: 현재 인물이 spouseId 쪽에 등록된 경우 — rel.person이 실제 배우자
          ...(person.spouseRelationsAsSpouse || []).map((r: any) => mapRel(r, r.person)),
        ]) {
          const spouseId = rel.spouse?.id ? String(rel.spouse.id) : null
          if (!spouseId) {
            noSpouseRows.push(rel)
            continue
          }
          const kept = bySpouseId.get(spouseId)
          if (!kept || relRichness(rel) > relRichness(kept)) bySpouseId.set(spouseId, rel)
        }
        return [...bySpouseId.values(), ...noSpouseRows]
          // 안정 정렬: 서열(정실 우선, 미분류 후순위) → 혼인 시작 부호 연도 → 행 id.
          // DB 자연 순서(UUID) 의존을 서버에서 제거해 모든 소비자가 같은 순서를 본다.
          .sort((a, b) => {
            const rankA = a.marriageRank ? (MARRIAGE_RANK_ORDER[a.marriageRank] ?? 99) : 99
            const rankB = b.marriageRank ? (MARRIAGE_RANK_ORDER[b.marriageRank] ?? 99) : 99
            if (rankA !== rankB) return rankA - rankB
            const yearA = signedStartYear(a)
            const yearB = signedStartYear(b)
  ...
  async create(@Body() dto: CreatePersonDto, @Request() req: any): Promise<PersonResponseDto> {
    ...
    return this.personService.create({
      ...
      spouseRelations: dto.spouseRelations?.map((s) => {
        // 구조화 값 우선, 레거시 ISO는 문자열 직접 파싱 — 행 단위 full-replace라 값 없음 = null 클리어
        const start = mapSpouseDateInput(s.marriageStart, s.marriageStartDate)
        const end = mapSpouseDateInput(s.marriageEnd, s.marriageEndDate)
        return {
          spouseId: s.spouseId,
          marriageStartDate: start.date ?? undefined,
          marriageStartEra: start.era,
          marriageStartYear: start.year,
          marriageStartMonth: start.month,
          marriageStartDay: start.day,
          marriageStartPrecision: start.precision,
          marriageEndDate: end.date ?? undefined,
          marriageEndEra: end.era,
          marriageEndYear: end.year,
          marriageEndMonth: end.month,
          marriageEndDay: end.day,
          marriageEndPrecision: end.precision,
          marriageRank: s.marriageRank ?? null,
          note: s.note,
        }
      }),
