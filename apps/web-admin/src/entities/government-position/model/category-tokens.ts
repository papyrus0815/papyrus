...
/**
 * 카테고리별 시각 토큰 — 수장비교 막대·범례·동시대 패널과 인물 상세 동시대 수장
 * 스트립이 모두 이 한 파일에서 색·라벨을 받아 일관성 유지.
 * (원래 widgets/heads-of-state-timeline/lib에 있던 것을 크로스-위젯 소비가 생기며
 *  entities로 승격 — 위젯 쪽 원경로는 re-export shim으로 무변경.)
 *
...
export const CATEGORY_TOKENS: Record<PositionCategory, CategoryToken> = {
  MONARCH: {
    label: '군주·왕족',
    glyph: '♛',
    bar: {
...
export function categoryBarStyle(category: PositionCategory) {
  const token = CATEGORY_TOKENS[category]
  return {
    light: css`
      background: ${token.bar.light.background};
      border-color: ${token.bar.light.border};
      color: ${token.bar.light.color};
    `,
    dark: css`
      background: ${token.bar.dark.background};
      border-color: ${token.bar.dark.border};
      color: ${token.bar.dark.color};
    `,
  }
}

export function categoryGlyph(category: PositionCategory): string {
  return CATEGORY_TOKENS[category].glyph
}

export function categoryLabel(category: PositionCategory): string {
  return CATEGORY_TOKENS[category].label
}
