...
- `widgets/.../government-info-section.widget.tsx` — 현재 **행정기구** 탭만 `<GovernmentOrganizationsTab />`로 feature 연동; 통계·중앙부처는 위젯 파일과 별도로 인라인 JSX가 남아 있음(추후 `GovernmentStatisticsTab` / `MinistriesTabSection` 등으로 치환 가능)

## 다음 단계 (선택)

1. **통계·중앙부처 탭** — `GovernmentInfoSection`에서 인라인 통계·부처 UI를 `government-statistics-tab.widget.tsx` · `ministries-tab-section.widget.tsx`로 교체해 단일 소스 유지
2. **국가 상세 다크모드 팔레트** — `country-detail-palette.ts`: `getCabinetsSectionPalette`, `getMapRegionSectionPalette`, **`getTreatySectionPalette`**. 행정부·조약·지도·**`GovernmentInfoSection`**·**`MinistryDepartmentTree`**·**`TlItem`(타임라인)**·**`department-blocks`**(재임·기관 사건) 등이 동일 팔레트를 사용합니다.
3. **행정부처 관리 페이지** — `useQuery` + `administrationDepartmentsAllQueryKey` / 무효화(`invalidateAdministrationDepartmentQueries`)로 국가 상세·목록·폼 캐시 정합성 유지 (적용됨)
