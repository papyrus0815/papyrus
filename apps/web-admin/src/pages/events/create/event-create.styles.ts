/**
 * 사건 등록 폼 스타일 — 페이지 셸과 `EventBasicForm`(모달·페이지 공용 본체)이 함께 쓴다.
 *
 * ## `shared/ui/register-form-layout`으로 합치지 않는 이유 (실측 근거, 2026-08-02)
 *
 * 그 kit이 "정본"이고 여기가 위반이라는 인상이 있었지만, 실제로 대조해 보면 **골격 컴포넌트가
 * 하나도 일치하지 않는다** — 같은 것의 사본이 아니라 서로 다른 시각 언어다:
 *
 * | | 이 파일 | register-form-layout |
 * |---|---|---|
 * | 라벨 컬럼 | 200px (1024px에서 160px) | 360px |
 * | 라벨 글자 | 14px | 13px |
 * | 필수 표식 | 빨간 `*` 글리프 | 5px 빨간 **점**(sr-only 텍스트) |
 * | 에러 | `⚠` 접두 + flex, #ef4444 | 평범한 block, #ea4335 |
 * | 힌트 | `text.muted`, line-height 1.5 | `text.secondary`, margin-top 6px |
 *
 * 특히 라벨 360px를 그대로 받으면 등록 모달(콘텐츠 990px) 안에서 필드가 606px로 눌려
 * **설계폭 680px 아래로 내려간다** — 모달 폭을 1040px로 잡아 확보한 "페이지 대비 가로 손실
 * 0"이 무너진다.
 *
 * 즉 이관은 리팩터가 아니라 **이 폼의 시각 언어를 통째로 바꾸는 재디자인**이다. 반쪽만
 * 옮기면 한 폼 안에 점 필수표식과 별표 필수표식이 섞인다. 진짜 canon 위반이었던 브랜드색
 * (이 폼만 앱의 *secondary*를 primary로 썼다)은 `getC`에서 이미 해소했다.
 *
 * 합치려면 두 폼의 시각 규격을 먼저 하나로 정한 뒤 kit을 그 규격으로 고치는 순서여야 한다.
 */
