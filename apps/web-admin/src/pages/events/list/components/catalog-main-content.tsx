 ...
 * - 상세 패널 호스트(>=1200px column, <1200px slide-in drawer) + backdrop
 */
import React from 'react'

import { FiBarChart2, FiList, FiX } from 'react-icons/fi'
...
interface Props {
  ...
  detailPanelSlot: React.ReactNode
}
...
export const CatalogMainContent: React.FC<Props> = ({
  ...
  return (
    <>
      ...
      {/* ===== 상세 패널 — 1200px 미만에선 drawer ===== */}
      <PageStyles.DetailDrawerBackdrop
        $open={!!selectedEventId}
        onClick={clearSelectedEvent}
        aria-hidden="true"
      />
      <PageStyles.DetailPanelHost $open={!!selectedEventId}>
        {selectedEventId && (
