...
export const TopFilterBar = styled.div.attrs(
  ...
  padding: 6px 0 10px;
  /*
   * (제거) border-bottom — 바로 아래 표의 열 머리글이 이미 같은 굵기의 선을 긋고 있어,
   * 8px 간격으로 가로선이 두 줄 겹쳐 있었다. 도구줄과 표 머리가 각자 '머리'를 주장하는
   * 모양이라, 지면 상단이 필요 이상으로 무겁게 읽혔다(사용자 판정 2026-09-24).
   * 경계는 표가 긋는다 — 도구줄은 그 위에 떠 있는 컨트롤 묶음이다.
   */
   ...
   * 원리상 맞출 수 없다.
   *
   * 1080은 셋이 **한 줄에** 서는 최소 폭이다: 검색 240 + 필터 453 + 액션 358 + 간격 20.
   * 액션 358 = 북마크 74 · 최상위 104 · 구분선과 표시 설정 45 · CTA 115 + 간격.
   * 이 아래로는 3존을 유지해 봐야 액션이 안에서 두 줄이 되므로 통째로 wrap이 낫다.
   *
   * ⚠️ 임계는 1220이었다. 도구줄에서 '최근'(64px)과 '계층'(82px)이 빠지고 필터 그룹이
   * 543 → 453으로 줄면서 같은 조판이 140px 좁은 폭에서도 선다 — 임계를 그대로 두면
   * 한 줄로 설 수 있는 대역에서 공연히 세 줄로 접혔다. 컨트롤을 더하거나 빼면
   * **이 숫자를 다시 유도할 것**(위 합계가 그 유도 과정이다). */
  @container catalogtoolbar (max-width: 1080px) {
    display: flex;
    gap: 10px;
    flex-wrap: wrap;
  }
