...
export const LIST_DENSITY = {
  compact: {
    ...
    discBtn: 24,
    /**
     * 시작·종료 두 열이 함께 쓰는 폭(--track-end가 이 값을 읽는다).
     *
     * 이 열의 최장 토큰은 'YYYY.M.D'(예 '1018.10.30')이고, 숫자가 tabular-nums라
     * 자릿수만큼 정직하게 넓다 — 12px에서 68px, 11px에서 62px. 예전 값(62/66/70)은
     * **자기 문법을 못 담는 폭**이었다: 시작이 일(日)을 잘라 '(1909.4)'로 찍어 겨우
     * 맞췄는데도 종료 열은 1384px에서 '1018.12.31'이 67px로 2행 넘쳤다(실측).
     * 시작이 종료와 같은 문법을 쓰게 되면서 폭도 그 문법에 맞춘다.
     */
    colDate: 66,
    colChip: 56,
    ...
    indent: 20,
    /**
     * 연 머리글의 세로 예산 — **모든 연 그룹**이 이 값을 쓴다.
     *
     * 예전엔 34+22+6 = 62px이었고, 그 값이 비싸서 1행짜리 그룹(119개 중 68개)의
     * 머리글을 통째로 지웠다. 그 처방이 "867년 아래에 895년 사건이 같이 나온다"를
     * 만들었다 — 라벨 없는 그룹이 위 밴드의 연장으로 읽힌 것이다.
     * 그래서 지우는 대신 **줄인다**: 24+12+2 = 38px. 머리글 51→119개로 늘려도
     * 총 세로는 +7% 안쪽이고(실측 19,976px 기준), 모든 해가 자기 이름을 갖는다.
     */
    yearH: 22,
    /** 연 헤더 라벨 — 행 제목(13px)보다 **작다**(아래 cozy 주석 참고). */
    yearLabel: 12,
    yearMt: 10,
    yearMb: 2,
    centuryH: 30,
  ...
  cozy: {
     ...
     */
    yearLabel: 13,
    /* 모든 연 그룹이 머리글을 갖는다 — 62px → 38px(24+12+2). 위 compact 주석 참고. */
    yearMt: 12,
    yearMb: 2,
    centuryH: 36,
  ...
  roomy: {
    ...
    indent: 32,
    yearH: 26,
    /** 연 헤더 라벨 — 행 제목(15px)보다 **작다**(cozy 주석 참고). */
    yearLabel: 14,
    /* 모든 연 그룹이 머리글을 갖는다 — 위 compact 주석 참고. */
    yearMt: 14,
    yearMb: 4,
    centuryH: 40,
    /** 세기 헤더 라벨 크기 — 연 헤더보다 한 단 위. */
...
export const SPAN_GRID = {
  /*
   * ⚠️ 위 규약("행 괘선보다 아래")이 **값에서는 지켜지지 않고 있었다** — 격자 0.07 대
   * 행 괘선 0.08은 지각상 같은 선이라, 기간 열의 세로 격자 셋이 행 구분선과 같은 무게로
   * 목록 한가운데를 세로로 관통했다. 이 열이 싣는 값은 대부분 점 하나(막대 중앙값 9px)인데
   * 바탕이 값만큼 진하면 바탕이 먼저 읽힌다.
   * 한 단 내려 규약을 값으로 집행한다(다크는 행 괘선을 0.12로 올리며 이미 벌어졌지만,
   * 같은 이유로 함께 내려 두 테마의 위계 폭을 맞춘다).
   */
  light: {
    year: 'rgba(15, 23, 42, 0.05)',
    quarter: 'rgba(15, 23, 42, 0.03)',
  },
  dark: {
    year: 'rgba(255, 255, 255, 0.055)',
    quarter: 'rgba(255, 255, 255, 0.035)',
  },
} as const
...
export const LIST_STEPS = {
  ...
  summary: 1180,
  /**
   * 7→8트랙. 키워드 열이 신축 트랙이 되고 기간·관련국이 넓어진다.
   *
   * 1752였다. 그런데 **1920 모니터의 카드 폭이 1750.8px**이다(사이드바 기본 접힘 기준:
   * 뷰포트 1925 − 아이콘 레일 48 − 접힌 목록 48 − PageWrapper 거터 2×) — 임계에
   * **1.2px 미달**이라, 가장 흔한 대형 데스크톱에서 이 단계가 한 번도 켜지지 않았다.
   * 위 주석이 "임계 바로 위 뷰포트에서 열이 안 켜지는 과거 회귀"라고 경고한 바로 그 형태다.
   *
   * 1752라는 값 자체가 유도값이 아니라 **여유를 얹은 값**이었다(유도식 합계는 1490,
   * 주석에도 "여유를 둬 1752"라고 적혀 있다). 그 임의의 여유 262px이 1.2px 때문에
   * 한 해상도 전체를 아래 단계에 묶어 둔 셈이다. 1740으로 내리면 1920 대역이 약 11px
   * 여유로 들어오고, 유도 최소치(1490)보다는 여전히 250px 위다.
   *
   * ⚠️ 카드 폭을 바꾸는 것(사이드바 폭·거터·스크롤바)은 이 숫자와 **같은 커밋에서**
   * 재확인할 것. 1px 차이가 해상도 하나를 통째로 가른다는 것이 이 값의 교훈이다.
   */
  ledger: 1740,
