...
export const DASHBOARD_MENU_ITEMS: DashboardMenuItem[] = [
  {
    ...
    label: '인물',
    icon: IconPeople,
    path: pathKeys.personsTimeline(),
    matchPath: (p) => /^\/persons-timeline(\/.*)?$/.test(p),
  },
  {
    id: 'legislature',
    label: '저원',
    icon: IconLegislature,
    path: pathKeys.legislatureTimeline(),
    matchPath: (p) => /^\/legislature\/?$/.test(p),
  },
  {
    id: 'military',
    label: '군사',
    icon: IconMilitary,
    path: pathKeys.militaryTimeline(),
    matchPath: (p) => /^\/military\/?$/.test(p),
  },
  {
    id: 'dynasty',
    label: '가문',
