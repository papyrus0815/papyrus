...
function getPositionType(t: any): string | null {
  return t?.positionType ?? t?.positionDefinition?.positionType ?? null
}

function getPositionTitle(t: any): string | null {
  return t?.positionDefinition?.title ?? t?.title ?? null
}

function getPersonName(t: any): string | null {
  const p = t?.person
  if (!p) return null
 ...
 */
function categorize(t: any): PositionTypeCategory {
  ...
  if (pt === 'HEAD_OF_STATE') {
    const method = (t?.appointmentMethod ?? '') as string
    if (
      method === 'HEREDITARY' ||
      method === 'SUCCESSION' ||
      method === 'COUP'
...
function ordinalOf(t: any): number | null {
...
function isSamePersonStart(a: any, b: any): boolean {
...
export function normalizeTenures(rawList: unknown): TenureBar[] {
  ...
  const tenures = rawList.filter((r: any) => r?.recordKind !== 'SOVEREIGN_REIGN')
  const sovereigns = rawList.filter((r: any) => r?.recordKind === 'SOVEREIGN_REIGN')

  // SovereignReign이 있으면 동일 시기 TENURE 제거
  const filteredTenures = tenures.filter(
    (t: any) => !sovereigns.some((s: any) => isSamePersonStart(t, s)),
  )
