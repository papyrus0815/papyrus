...
function buildInitialStatDraft(
  stats: PersonStats | null | undefined,
): StatDraft {
  ...
  const saveMut = useMutation({
    mutationFn: () =>
      upsertMyEvaluation(personId, {
        ...
        traits: {
          items: Array.from(draftTraits.entries()).map(
            ([trait, intensity]) => ({
              trait,
...
const IntensityDot = styled.button<{
  ...
  color: ${({ $active, $tone }) => ($active ? '#fff' : traitToneBg($tone).fg)};
